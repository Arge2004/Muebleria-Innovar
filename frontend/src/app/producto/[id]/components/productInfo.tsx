...
import { Product } from '@/app/landing/data/products';
...
const ProductInfo: React.FC<ProductInfoProps> = ({ product, onBuyClick }) => {
  return (
    <div className="order-3 space-y-4">
      ...
      <button
        onClick={onBuyClick}
        className="w-full py-3 bg-[#B8CDA2] hover:bg-[#A9B997] text-black cursor-pointer font-normal rounded flex items-center justify-center gap-2 transition-colors"
      >
        <FaWhatsapp size={20} />
        Comprar ahora
      </button>
      ...
      <div className="space-y-2 pt-2">
        ...
        <Accordion title="Detalles">
          <div className="text-gray-700 text-sm space-y-2">
            <p><strong>Categoría:</strong> {product.category.charAt(0).toUpperCase() + product.category.slice(1)}</p>
            <p><strong>Dimensiones:</strong> Variable según modelo</p>
            <p><strong>Peso:</strong> Consultar con asesor</p>
          </div>
        </Accordion>
